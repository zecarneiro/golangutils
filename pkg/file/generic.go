...
func Move(src string, dstBaseDir string) error {
	...
	if common.HasLastChar(src, "/") {
		src = common.DeleteLastChar(src)
	} else if common.HasLastChar(src, "\\") {
		src = common.DeleteLastChar(src)
	}
	if common.HasLastChar(dstBaseDir, "/") {
		dstBaseDir = common.DeleteLastChar(dstBaseDir)
	} else if common.HasLastChar(dstBaseDir, "\\") {
		dstBaseDir = common.DeleteLastChar(dstBaseDir)
	}
	if !FileExist(src) {
