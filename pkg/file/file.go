...
func WriteFileWithEncoding(file string, data string, isAppend bool, isCreateDir bool, encodingName string) error {
	...
	encodingName = common.Ternary(len(encodingName) > 0, encodingName, utfName)
	encodingName = common.Ternary(encodingName == "UTF-8", utfName, encodingName)
	if isCreateDir {
...
func ReadJsonFile[T any](jsonFile string) (T, error) {
	...
	return common.StringToObject[T](data)
}
...
func WriteJsonFile[T any](jsonFile string, data T, escapeHtml bool) error {
	...
	var err error
	if escapeHtml {
		dataStr, err = common.ObjectToStringEscapeHtml(data)
		if err != nil {
			return err
		}
	} else {
		dataStr, err = common.ObjectToString(data)
		if err != nil {
			return err
		}
	}
