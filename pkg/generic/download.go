...
func (pw *progressWriter) Write(p []byte) (int, error) {
	...
	if pw.Total > 0 {
		...
		fmt.Printf("\rProgress: %.2f%% | Total size: %.2f MB", percent, common.BytesToMB(pw.Total))
	} else {
		// Se o servidor não informar o tamanho, mostra apenas o que já baixou
		fmt.Printf("\rDownloading: %.2f MB (Unknown total size)", common.BytesToMB(pw.Downloaded))
	}
	return n, nil
}
