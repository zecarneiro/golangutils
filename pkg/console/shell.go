...
func IsShell(shells []ShellType) bool {
	return common.InArray(shells, GetCurrentShell())
}

func BuildBashCmd(cmd string, args []string) entity.Command {
...
func BuildPowershellCmd(cmd string, args []string) entity.Command {
	command := entity.Command{
		Cmd:  "powershell.exe",
...
func BuildPromptCmd(cmd string, args []string) entity.Command {
	command := entity.Command{
		Cmd:  "cmd.exe",
		Args: append([]string{"/C", cmd}, args...),
	}
